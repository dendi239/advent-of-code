...
BY_STRENGTH = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
...
def canonical_hand2(hand: str) -> str:
    return canonical_hand(pick_jockers(hand))[:1] + canonical_hand(hand)[1:]


def pick_jockers(hand: str) -> str:
    indices = [i for i in range(len(hand)) if hand[i] == "J"]
    ...
    return max(
        map(
            replace, itertools.combinations_with_replacement(BY_STRENGTH, len(indices))
        ),
        key=canonical_hand,
    )
...
def main() -> None:
    ...
    with open(args.input) as f:
        ...
        hand_bids = sorted(hand_bids, key=lambda x: canonical_hand2(x[0]))
